...
@section('title','home')
@section('content')
    <h2 class="d-flex justify-content-center mt-4">Add Game</h2>
    <div class="container p-2 w-50">
    <form method="POST" action="{{route('games.create')}}" class="form" enctype="multipart/form-data">
        @csrf
        ...
        <div class="form-group p-2">
            ...
            <input type="number" class="form-control bg-muted" id="price" name="price">
            @error('price')
            <small class="error text-danger">{{$message}}</small>
            @enderror
        </div>
        ...
        <div class="form-group-button p-3 d-flex justify-content-end">
            <button type="submit" class="form-button btn-primary rounded-pill px-3 py-2">Add</button>
        </div>
    </form></div>
